...
from utils.schemas import TrainedModel, SampleInfo
...
class AgenticAIPipeline:
    ...
    def run_prepare_data_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            sample_records = []
            import json
            with open("tmp/data_from_gcs_info.json", "r") as f:
                sample_records_json = json.load(f)
                for rec in sample_records_json:
                    sample = SampleInfo(**rec)
                    sample.bucket_name = rec['img_path'].split('/')[2]
            ...
            status, message = push_file_to_gcs(json_tmp_file_path, self.config.data_pipeline['output_configs']['gcs_destination'])
            if not status:
                return {"status": "failed", "error": message}
            status, message = push_new_samples_to_label_studio(url=self.config.LABEL_STUDIO_URL, api_key=self.config.LABEL_STUDIO_API_KEY, project_id=self.config.label_studio_configs['project_id'], path_to_new_samples_json=json_tmp_file_path)
            if not status:
                return {"status": "failed", "error": message}
            return {"status": "Run prepare data pipeline successfully"}
