"""
Extract confirmed False Positive detections of a given RFDETR model/month
into cropped image patches + a manifest, for manual visual review.

A detection is a "confirmed FP" when its bbox matches (IoU >= iou-threshold)
a ground_truth box whose worker-corrected label is "Khong_co_loi" (i.e. the
worker looked at that exact spot and confirmed there was no defect there).
Matching is class-aware: the detection's class must belong to the same
defect family as GT boxes it's being scored against would (see CLASS_MAP) -
this only matters for the "real defect" TP/FN side; FP matching itself does
not require a class match since Khong_co_loi has no defect family.
...
Writes:
    ...
    <output>/manifest.json                        - one entry per extracted FP
    <output>/gallery.html                         - self-contained viewer (grid of
        crops, sorted by confidence; click a card to see the FP box drawn on the
...
<head>
<meta charset="utf-8">
<title>False Positives Viewer</title>
<style>
  ...
  .card .conf { font-weight: 600; color: #e74c3c; }
  ...
  svg.overlay rect { fill: none; stroke: #e74c3c; stroke-width: 3; }
  .modal-meta { position: absolute; bottom: -36px; left: 0; color: #ccc; font-size: 13px; }
</style>
</head>
<body>
<header>
  <h1>False Positives Viewer <span class="count" id="count"></span></h1>
  <div class="controls">
...
def is_confirmed_fp(det: dict, ground_truth: list, iou_threshold: float) -> tuple:
    """Returns (is_fp, matched_gt) - matched against ANY gt box regardless of
    class (location-only), same convention as scripts/compute_metrics.py."""
    best_iou, best_gt = 0.0, None
    for g in ground_truth:
        score = iou(det["bbox"], g["bbox"])
        if score > best_iou:
            best_iou, best_gt = score, g
    if best_iou >= iou_threshold and best_gt is not None and best_gt["class"] == NO_DEFECT_LABEL:
        return True, best_gt
    return False, None
...
def extract(results_dir: str, model: str, month: str, output_dir: str,
    ...
    for json_path in sorted(Path(results_dir).rglob("*.json")):
        ...
        for idx, det in enumerate(detections):
            ...
            if det["confidence"] < min_confidence:
                continue
            is_fp, matched_gt = is_confirmed_fp(det, ground_truth, iou_threshold)
            if not is_fp:
                continue
    ...
    with open(output_path / "gallery.html", "w") as f:
        f.write(GALLERY_HTML.replace("__MANIFEST__", json.dumps(manifest, ensure_ascii=False)))

    logger.info(
        f"Scanned {n_records_matched_month} records in month={month!r}, "
        f"{n_detections_seen} {model} detections seen, "
        f"{len(manifest)} confirmed FP extracted to {crops_path}"
    )
    return manifest


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Extract confirmed FP detections as image crops")
    ...
    parser.add_argument("--output", default="reports/false_positives", help="Output directory for crops + manifest")
...
def main():
    ...
    manifest = extract(
        args.results, args.model, args.month, args.output,
        args.iou_threshold, args.padding, args.min_confidence,
    )

    print(f"Extracted {len(manifest)} confirmed FP crops to {args.output}/crops")
    print(f"Manifest: {args.output}/manifest.json")
    print(f"Viewer: {args.output}/gallery.html (serve together with --results, e.g. `python -m http.server --directory .`)")
    return 0
