...
contains at least one "ignore" class box into a separate project - no model
inference involved, this is a pure human-annotation filter.
...
Usage:
    python scripts/move_ignore_tasks.py --source-project-id 23 --target-project-id 25

    # Dry-run: report which tasks would move, without importing/deleting anything
    python scripts/move_ignore_tasks.py --source-project-id 23 --target-project-id 25 --dry-run
"""
...
def has_ignore_class(human_annos: list) -> bool:
    return any(_canonical(a["label"]) == IGNORE_LABEL for a in human_annos)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Move tasks containing an 'ignore' class annotation into a separate project",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--source-project-id", type=int, required=True, help="Label Studio project ID to pull tasks from")
    parser.add_argument("--target-project-id", type=int, required=True, help="Label Studio project ID to move ignore tasks into")
...
def main():
    ...
    for i, task in enumerate(tasks, 1):
        ...
        if not has_ignore_class(sample["annos"]):
            kept += 1
            continue

        logger.info(
            f"[{i}/{len(tasks)}] task {task_id}: contains 'ignore' class, "
