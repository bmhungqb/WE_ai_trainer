"""
Move every negative sample (a reviewed/annotated task with zero defect
boxes - same definition as scripts/pull_label_studio_samples.py's default
negative-sample exclusion) from a Label Studio project into a separate
project - no model inference involved, this is a pure human-annotation
filter.
...
i.e. the empty "no defects" annotation) into the target project via
...
Usage:
    python scripts/move_negative_tasks.py --source-project-id 23 --target-project-id 25

    # Dry-run: report which tasks would move, without importing/deleting anything
    python scripts/move_negative_tasks.py --source-project-id 23 --target-project-id 25 --dry-run
"""
...
def is_negative_sample(sample: dict) -> bool:
    """Reviewed (process_task returned a sample) but with zero defect boxes -
    same convention as pull_label_studio_samples.py's default exclusion."""
    return not sample["annos"]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Move negative samples (reviewed tasks with no defect annotations) into a separate project",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--source-project-id", type=int, required=True, help="Label Studio project ID to pull tasks from")
    parser.add_argument("--target-project-id", type=int, required=True, help="Label Studio project ID to move negative samples into")
...
def main():
    ...
    for i, task in enumerate(tasks, 1):
        ...
        if not is_negative_sample(sample):
            kept += 1
            continue

        logger.info(
            f"[{i}/{len(tasks)}] task {task_id}: negative sample (no defect boxes), "
